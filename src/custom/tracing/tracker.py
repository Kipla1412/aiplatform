...
class RAGTracer:
    ...
    @contextmanager
    def trace_generation(self, trace, model: str, prompt: str):
        """LLM generation tracked as Langfuse generation (not just span)."""
        generation = self.tracer.create_generation(
            trace=trace,
            name="llm_generation",
            model=model,
            input_data={"prompt": prompt, "prompt_length": len(prompt)},
        )
        try:
            yield generation
        finally:
            pass  # generation is completed when output is logged
