...
from .langfusetracer import LangfuseTracer
...
class RAGTracer:
    ...
    def trace_request(self, user_id: str, query: str):
        """Main request trace context managet."""
    ...
    def score_answer(self, trace, relevance: float):
        """Attach a relevance score to the trace."""
        if not trace:
            return
        self.tracer.score_trace(trace, name="relevance", value=relevance)
...
#     @contextmanager
#     def trace_embedding(self, query: str):
#         start_time = time.time()
...
#                 self.tracer.update_span(span=span, level="ERROR", status_message=str(e))
#                 raise

#     def end_search(self, span, chunks, arxiv_ids, total_hits):
#         self.tracer.update_span(
#             span=span,
#             output={
#                 "chunks_returned": len(chunks),
...
#     @contextmanager
#     def trace_prompt_construction(self, chunks):
#         with self.tracer.start_span(
...
#     @contextmanager
#     def trace_generation(self, model: str, prompt: str):
...
#     def end_generation(self, generation, response: str, usage: Optional[Dict[str, Any]] = None):
...
#     def score_answer(self, relevance: float):
#         trace_id = self.tracer.get_trace_id()
#         if trace_id:
#             self.tracer.submit_feedback(trace_id=trace_id, score=relevance, name="relevance")
