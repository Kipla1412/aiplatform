...
from src.custom.agent.context.arxivcontext import Context
from src.custom.agent.config.arxivconfig import GraphConfig
from src.custom.agent.schemas.state import AgentState

from src.custom.agent.nodes import (
...
from src.custom.agent.tools.retrievetool import  create_retriever_tool
...
class AgenticRAGService:
    ...
    async def ask(
        ...
        from src.custom.llm.schemas.redisschema import AskRequest
        ...
        request_obj = AskRequest(
            ...
            top_k=self.graph_config.top_k,
            use_hybrid=self.graph_config.use_hybrid,
            categories=None,  # or pass real categories if you use them
        )

        # CHECK REDIS CACHE FIRST
        if self.cache_client:
            cached_response = await self.cache_client.find_cached_response(request_obj)
            if cached_response:
                logger.info("CACHE HIT — returning response from Redis")
        ...
        async def _execute_with_trace():
            ...
            if trace is not None:
                with trace as trace_obj:
                    ...
                    return await self._run_workflow(query, model_to_use, user_id, trace_obj, request_obj)
            else:
                return await self._run_workflow(query, model_to_use, user_id, None, request_obj)
    ...
    async def _run_workflow(self, query: str, model_to_use: str, user_id: str, trace, request_obj) -> dict:
        ...
        try:
            ...
            if self.cache_client:
                try:
                    from src.custom.llm.schemas.redisschema import AskResponse
                    response_obj = AskResponse(**final_response)
                    await self.cache_client.store_response(request_obj, response_obj)
                    logger.info(f"Cached response for model={model_to_use}")

                except Exception:
                    logger.exception("Failed to store response in cache")
